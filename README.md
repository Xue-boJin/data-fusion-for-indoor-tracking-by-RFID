...
#### 例 蛇型机动的正弦曲线模拟 
给出函数funtrackingsnake，里面包括四个输入变量，其中a是正弦曲线的振幅，omig指正弦曲线的角加速度，和前面一样，t表示从开始到结束由每个采样点组成的采样时刻向量，R是测量噪声的方差。同样，输出变量有两个，一个是不带测量噪声的yreal，一个是带有测量噪声的ym。 function [yreal,ym]=funtrackingsnake(a,omig,t,R) yreal=asin(omigt); ym=yreal+randn(size(yreal))*sqrt(R);
